...
public class Quicksort implements Sorter {
  ...
  /**
   * Recursively divides the array being sorted into subarrays that will be sorted.
   * 
   * @param values The original array being sorted.
   * @param compare The comparator that determines what order the elements in the array 
   *                will be sorted in.
  ...
  private static <T> void quickSort(T[] values, Comparator<? super T> compare, int lb, int ub) {
    ...
    if (ub - lb <= 1) {
      ...
      // Otherwise, keep partitioning the array recursively into smaller subarrays 
      // that will then be sorted
  ...
  /**
   * Select a pivot, partition the subarray from [lb .. ub), and sort that subarray.
   * 
   ...
   * @pre lb and ub are valid indices of the subarray in consideration (from 0 to arr.length - 1)
   */
  private static <T> int partition(T[] arr, Comparator<? super T> order, int lb, int ub) {
    // Determine the pivot by taking the median of the lower bound, upper bound, and the 
    // index that is the middle of the lower and upper bounds
    ...
    int small = lb + 1; // Excluding the element at the pivot location (which is at the very front)
    int large = ub;

    // Note: Elements smaller than or equal to the pivot stay on the left side of the subarray 
    // while elements greater than the pivot stay on the right side of the array
    while (small < large) { // Continue while 
      if (order.compare(arr[small], arr[lb]) <= 0) {
        // If the element on the left side of the subarray is less than or equal to the 
        // pivot, the former stays on the left side, so move the barrier location, small, 
        // to the right by 1
        small++;
      } else if (order.compare(arr[large - 1], arr[lb]) > 0) { // large - 1 b/c ub is exclusive
        // If the element on the right side of the subarray is greater than the pivot, 
        // the former stays on the right side, so move the barrier location, large, 
        // to the left by 1
        large--;
      } else {
        // Otherwise, the element on the left side of the subarray is greater than the pivot and 
        // the element on the right side of the subarray is less than or equal to the pivot, so 
        // the two elements should be swapped to end up in the correct sections of the subarray.
        // The barriers for both small and large are moved over, so small moves to the right by 
        // 1 after the swap, and large moves to the left by 1 before the swap 
        swap(arr, small, --large);
        small++;
      }
    }

    // Finally, swap the pivot to the position right after the last smaller element so the pivot
    // can separate the sections of elements smaller than it and greater than it
    swap(arr, lb, small - 1);
  ...
  private static int medianOfThree(int left, int right) {
    int middle = left + (right - left) / 2; // find the middle of the left and right
    
    if (left > middle && left > right) {
      // left is largest, so determine the next largest
      if (middle >= right) {
        return middle; // middle is median (as it's the second largest)
      } else {
        return right; // right is median (as it's the second largest)
      }
    }
    ...
    if (middle > left && middle > right) {
      ...
      if (left >= right) {
        return left; // left is median (as it's the second largest)
      } else { 
        return right; // right is median (as it's the second largest)
      }
    }
